...
int iEEVExtruder = 1;				//  Datensatz aus EEPRom Soll Geschwindigkeit des Extruders von der max Geschwindigkeit 100% = 1Hz Extruderschnecke 19Hz Extrudermotor
...
void b16PushCallback(void *ptr)		// Anwahl des Datensates +1
{
	...
	n12.getValue(&number);
	number++;
	iDatensatzNum = number;
	n12.setValue(number);
	
	LadenEEprom(iDatensatzNum);
	iEEMaterialart = Rezept[i][1];
	// Rezept Materialfarbe einfügen
	iEETempSollHeizblock = Rezept[i][2];
	iEETempSollDuese = Rezept[i][3];
	iEEVExtruder = Rezept[i][4];
